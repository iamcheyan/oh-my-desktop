...
Singleton {
    id: root
    readonly property string autoSavePrefix: "omd-session save-auto >/tmp/omd-session-auto-save.log 2>&1; "
    ...
    function logout(saveCurrentSession) {
        Quickshell.execDetached(["bash", "-lc", withOptionalSessionSave("omd-logout", saveCurrentSession)]);
    }
