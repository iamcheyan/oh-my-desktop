...
3. **toplevel 缩略图是 Hyprland 独占**（`hyprland-toplevel-export-v1`，Quickshell 唯一
   ...
   `ScreencopyView` 都出不了窗口缩略图——labwc 分支的缩略图方案（自写
   `sumika-overview-thumbnaild` daemon + `grim -T`，依赖
   `ext-foreign-toplevel-image-capture-source` + `ext-image-copy-capture`）在
   `labwc-adaptation` 分支。wlroots 库本身没有 toplevel-image-capture-source
   实现：labwc 0.20+ 是自实现（`src/server.c`），cosmic-comp 也实现了该协议，
   故方案在 labwc 0.20+ / cosmic-comp 可移植；**sway/wayfire 等纯 wlroots
   合成器默认没有**，KWin/Mutter/Weston 也没有（§3 矩阵）。
