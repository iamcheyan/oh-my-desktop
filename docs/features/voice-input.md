...
`$SUMIKA_SHELL_EXTENSIONS_DIR/voice/scripts/voice-bind-tui`. The capture tool
uses the keyboard-remap extension's `scripts/key-test --hotkey`, so it captures
the final key after keyd remaps rather than the physical source key.
...
## Key Capture Tool (keyboard-remap extension)
...
To resolve this, the keyboard-remap extension's `scripts/key-test --hotkey` is
written as a native **GTK4 / Libadwaita** application that runs in its own
Wayland client window, allowing it to capture the final key events seen by
applications after keyd remaps while styled with custom CSS to look like a
premium terminal utility. Keyboard Remap uses `--remap-source` instead when it
needs physical source keys.
...
Running the keyboard-remap extension's `scripts/key-test` without arguments
opens an in-window mode switch.
...
3. Voice bindings: run capture through
   `$SUMIKA_SHELL_EXTENSIONS_DIR/voice/scripts/voice-bind-tui`
   (`key-test --hotkey`).
...
- `$SUMIKA_SHELL_EXTENSIONS_DIR/keyboard-remap/scripts/key-test --hotkey` —
  GTK4 hotkey capture after keyd remaps
