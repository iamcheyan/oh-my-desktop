...
### [ ] 1. ResourceUsage: 1ms startup timer + spread/shift history buffers
...
### [ ] 2. DateTime: 10ms uptime timer reads /proc/uptime
...
### [ ] 3. TimerService: 10ms stopwatch timer drives a property binding
...
### [ ] 4. OverviewWidget: comma-operator `modelRevision` hack
...
### [ ] 5. SessionButton: 5s polling spawns `hyprctl -j clients | jq`
...
### [ ] 6. Network: nmcli monitor cascade + AP createObject churn
...
### [ ] 7. Notifications: per-notification createObject + full-list JSON.stringify
...
### [ ] 8. Notifications: `onListChanged` rebuilds `latestTimeForApp` from scratch
...
### [ ] 9. GlobalFocusGrab: `hasActive` recursively walks the UI tree
...
### [ ] 10. Hyprsunset: spawns processes on every gamma/temp change
...
### [ ] 11. MaterialSymbol: `Behavior on fill` leak + string-typed axis
...
### [ ] 12. OverviewSearch: opacity animation defeated by `visible`
...
### [ ] 17. CircularProgress: `Loader` for a toggleable Rectangle
