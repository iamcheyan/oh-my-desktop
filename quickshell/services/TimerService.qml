...
Singleton {
    ...
    // Pre-computed display strings so mm:ss only re-renders when the whole
    // second changes, not on every 10ms tick. centiDisplay still updates at
    // 100Hz but is a tiny label.
    property string stopwatchMainDisplay: "00:00"
    property string stopwatchCentiDisplay: "00"
    ...
    function refreshStopwatch() {  // Stopwatch stores time in 10ms
        ...
        stopwatchMainDisplay = `${minutes}:${seconds}`;
        stopwatchCentiDisplay = (Math.floor(stopwatchTime) % 100).toString().padStart(2, '0');
    }
    ...
    function stopwatchReset() {
        stopwatchTime = 0;
        stopwatchMainDisplay = "00:00";
        stopwatchCentiDisplay = "00";
