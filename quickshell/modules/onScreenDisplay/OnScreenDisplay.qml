...
Scope {
    ...
    Connections {
        ...
        function onBrightnessChanged() {
            // Don't show OSD if the display popup is already open (slider shows value).
            if (GlobalStates.barPopupType === "display")
                return;
            root.protectionMessage = "";
            // Prefer the monitor that was just adjusted (set by Brightness service).
            const monName = Services.Brightness.lastAdjustedScreenName || GlobalStates.osdBrightnessScreen || "";
            if (monName)
    ...
    Connections {
        ...
        function onVolumeChanged() {
            if (!ServiceManager.audio.ready)
                return;
            // Don't show OSD if the audio popup is already open (slider shows value).
