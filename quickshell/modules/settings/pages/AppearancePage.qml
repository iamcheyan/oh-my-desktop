...
ColumnLayout {
    ...
    QtObject {
        ...
        function apply(slug) {
            ...
            themeApplyProc.command = [
                "bash", "-c",
                `${FileUtils.trimFileProtocol(Directories.root)}/quickshell/modules/settings/bin/omd-settings-theme apply '${slug.replace(/'/g, "'\\''")}'`
            ]
        }
    }
    ...
    Process {
        command: ["bash", "-c", `${FileUtils.trimFileProtocol(Directories.root)}/quickshell/modules/settings/bin/omd-settings-theme list`]
        running: true
        stdout: StdioCollector {
    ...
    Process {
        command: ["bash", "-c", `${FileUtils.trimFileProtocol(Directories.root)}/quickshell/modules/settings/bin/omd-settings-theme current`]
        running: true
        stdout: StdioCollector {
